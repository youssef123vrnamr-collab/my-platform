...
async function isAdminUser(userId) {
  ...
  // مقطوع (finish_reason === 'length') زي ما بيحصل مع الرد الأصلي بالظبط. من غير الدالة
  // دي، أي مرحلة تصحيح لاحقة كانت بتكتب فوق الرد الكامل برد مقطوع من غير ما حد يلاحظ. ──
  // ══════════════════════════════════════════════════════════════════
  async function _cosmosFetchWithContinuation(apiKey, systemMsg, userMsg, maxTok) {
    var messages = [
    ...
    while (tries < 4) {
      ...
      if (!r.ok) return full || null;
      var d = await r.json();
      var piece = d.choices && d.choices[0] && d.choices[0].message && d.choices[0].message.content;
      var finishReason = d.choices && d.choices[0] && d.choices[0].finish_reason;
      if (!piece) return full || null;
      full += piece;
      if (finishReason !== 'length') break; // خلّص عادي، مش مقطوع
      messages.push({ role: 'assistant', content: piece });
      messages.push({ role: 'user', content: 'كمّل بالظبط من حيث وقفت من غير ما تعيد أي جزء اتكتب قبل كده ومن غير مقدمات.' });
    }
  ...
  window.deepenCosmosJsLogic = async function(apiKey, originalUserMsg, previousAnswer, coreLogicList) {
    try {
      ...
      var out = await _cosmosFetchWithContinuation(apiKey, sys, usr, _cosmosCorrectionTokenBudget(previousAnswer));
      return (out && out.length > 20 && /```/.test(out)) ? out : null; // ── لازم يفضل فيه كتل كود، وإلا نتجاهله (شكل اتغيّر) ──
  ...
  var WEB_SEARCH_TRIGGERS = [
    ...
    // ── كلمات حداثة عامة كانت ناقصة، وده كان بيخلي أسئلة زي "آخر رحلة فضاء" ماتتبحثش خالص ──
  ...
  window.classifyNeedsSearch = async function(userMsg){
    try {
      ...
      var r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
        ...
        headers: { 'Authorization': 'Bearer ' + apiKey, 'Content-Type': 'application/json' },
        body: JSON.stringify({
          model: 'openai/gpt-oss-120b', // ── رفعناه من الموديل الصغير (20b) للكبير عشان قرار "محتاج بحث ولا لأ" يبقى أدق، خصوصًا للأسئلة اللي مش صريحة ──
          max_tokens: 3,
          temperature: 0,
          messages: [
            { role: 'system', content: 'رد بكلمة واحدة بس: "نعم" لو الرسالة محتاجة معلومة حديثة/حقيقية أو حدث حالي أو حاجة لازم تتأكد منها من الإنترنت (زي أخبار، أسعار، تواريخ قريبة، أسماء أشخاص أو شركات أو منتجات حالية، نتائج، إحصائيات، حاجة بتتغيّر بمرور الوقت). خد بالك: أي سؤال فيه "آخر"، "أحدث"، "امتى/متى"، "لسه"، "حاليًا"، "دلوقتي" — زي "آخر رحلة فضاء" أو "مين حاليًا كذا" — يعتبر محتاج بحث ("نعم") حتى لو الموضوع علمي/فلكي عام، لأن الحدث الأحدث ممكن يكون اتغيّر عن اللي في معرفتك المخزّنة. أو رد "لا" لو مجرد كلام عادي، تحية، سؤال عن مفهوم علمي/تاريخي ثابت مالوش علاقة بأحدث حدث، طلب مساعدة عامة مش محتاجة تصفح إنترنت، أو طلب برمجة/كود/لعبة/موقع/تطبيق. رد بكلمة واحدة بس من غير أي شرح.' },
            { role: 'user', content: userMsg }
          ]
        })
      });
...
(function(){
  ...
  // escapeHtml بتستخدم النسخة العامة المعرّفة أول الملف (نفس السلوك بالظبط) — النسخة المحلية اتشالت لتجنب التكرار
...
(function() {
  ...
  // ── تعليمة مشتركة بتتضاف لكل الشخصيات: بتمنع الموديل إنه "يهلوس" ويدّعي إنه بعت طلب لنظام توليد
  ...
  const AI_PERSONAS = [
    {
      ...
      systemPrompt: 'أنت AlalaGyGyAgha V1.6، مساعد فلكي خبير ومتحمس. تجيب بدقة علمية وبشكل مشوّق. تتكلم بأي لغة يستخدمها المستخدم. أكمل ردك دائماً حتى النهاية ولا تتوقف في المنتصف.' + _NO_FAKE_IMAGE_CLAIM_SUFFIX,
    ...
    },
    {
      ...
      systemPrompt: 'أنت أوريون، معلم هادئ وصبور. تشرح الأمور ببساطة وبأمثلة واضحة. تتكلم بأي لغة يستخدمها المستخدم. أكمل ردك دائماً حتى النهاية ولا تتوقف في المنتصف.' + _NO_FAKE_IMAGE_CLAIM_SUFFIX,
    ...
    },
    {
      ...
      systemPrompt: 'أنت نوفا، مساعد ذكي وسريع. إجاباتك مباشرة ودقيقة ومكتملة. تتكلم بأي لغة يستخدمها المستخدم. أكمل ردك دائماً حتى النهاية ولا تتوقف في المنتصف.' + _NO_FAKE_IMAGE_CLAIM_SUFFIX,
    ...
    },
    {
      ...
      systemPrompt: 'أنت جالكسي، راوي قصص الفضاء. إجاباتك مليئة بالإثارة والتشويق والمعلومات الرائعة. تتكلم بأي لغة يستخدمها المستخدم. أكمل ردك دائماً حتى النهاية ولا تتوقف في المنتصف.' + _NO_FAKE_IMAGE_CLAIM_SUFFIX,
...
(function(){
  ...
  var iv = setInterval(async function(){
    ...
    window.sendAIMessage = async function(injectedMsg) {
      ...
      if (_needsSearch) {
        ...
        try {
          ...
          if (_searchRes && _searchRes.results && _searchRes.results.length) {
            ...
            _wctx += '---\nاستخدم النتائج الحقيقية دي عشان تتأكد من المعلومة وتجاوب بدقة. لو النتائج دي فيها حدث حديث بيغيّر أو بيكمّل معلومة كانت عندك من قبل (زي مهمة فضائية حصلت، أو حدث مستجد)، لازم تذكره في ردك ولو ماكنتش عارفه قبل كده — متجاهلش نتائج البحث وترجع لمعلوماتك القديمة المخزّنة. النتائج دي دايمًا أحدث وأولى بالاعتماد عليها من أي معلومة عندك من التدريب. لو في رابط حقيقي مناسب من النتائج دي، اذكره صريح في ردك. متخترعش أي رابط أو موقع مش موجود فوق.';
      ...
      // ── Persona & API key ──
      var persona  = window.getCurrentAIPersona() || { name:'Astronomy AI', emoji:'🔭', systemPrompt:'أنت مساعد متخصص في الفلك والفضاء. أجب بالعربية بدقة ووضوح. مهم جداً: معندكش أي قدرة تولّد أو تبعت طلب توليد صورة بنفسك — ممنوع تدّعي إنك "بعت الطلب" أو "الصورة قيد التحضير" لأن ده كذب، الاستدعاء الفعلي بيحصل بس لو المستخدم كتب كلمة صريحة زي "ارسم".' };
      var apiKey   = typeof getAiApiKey === 'function' ? getAiApiKey() : '';
      ...
      // ══════════════════════════════════════════════════════════════════
      // ── ملاحظة: فحص "وضوح الطلب" (checkBuildRequestClarity) اتعطّل هنا بطلب المستخدم —
      // كان بيوقف قبل أي كود ويسأل سؤال توضيحي حتى في حالات كتير كانت واضحة كفاية، فبقى حاسس
      // إن كوزموس "مبقاش بيفكر لوحده زي الأول". رجّعناه يبني على طول من أول رسالة زي ما كان. ──
      ...
      var _mandatoryAnyCodeBlock = '\n\nقاعدة إلزامية عامة لأي كود مهما كان نوعه: مش بس كود الويب (HTML/CSS/JS) — أي كود من أي نوع تكتبه في ردك (قواعد أمان Firestore، SQL، JSON، YAML، إعدادات، سكريبت بايثون، أو أي مقتطف كود مهما كان قصير) لازم يتحط جوه ثلاث علامات ``` مع اسم اللغة، بدون أي استثناء، حتى لو سطر واحد بس. ممنوع تمامًا يظهر أي كود كنص عادي في الرد.';
      var _codeFormatPolicyBlock = _mandatoryAnyCodeBlock + (_wantsSeparateFiles
      ...
      function buildPayload(model, maxTok, hist, lean) {
        ...
        return {
          ...
          max_tokens: maxTok,
          temperature: 0.4,
          // ── بطلب المستخدم: التفكير العميق ('high') بقى مفعّل حتى في طلبات الكود، عشان كوزموس
          // يخطط ويراجع منطقه صح قبل ما يكتب. ملحوظة: ده ممكن ياكل توكنز أكتر من ميزانية الكود نفسه
          // في الصفحات الضخمة جدًا (زيادة احتمال التقطيع) — لو حصل ده كتير، رجّعها 'medium' كحل وسط. ──
...
(function(){
  ...
  function _looksLikeCodeParagraph(text){
    ...
    // ── شبكة أمان إضافية: كود قواعد بيانات/إعدادات (Firestore rules، SQL، JSON، YAML) —
    // مالوش نفس بصمة JS/HTML/CSS، وكان بيفلت كنص عادي RTL من غير الإضافة دي ──
    if (/(^|\n)\s*(rules_version|service\s+cloud\.firestore|match\s+\/|allow\s+(read|write|create|update|delete))/i.test(t)) score += 2;
    if (/^\s*(SELECT|INSERT\s+INTO|UPDATE|DELETE\s+FROM|CREATE\s+TABLE|ALTER\s+TABLE)\b/im.test(t)) score += 2;
    if (/^\s*[{[][\s\S]*[}\]]\s*$/.test(t) && /"[\w-]+"\s*:/.test(t)) score += 2; // JSON خالص
    return score >= 2;
  }
