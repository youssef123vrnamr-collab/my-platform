...
async function updateAdminUI() {
  ...
  function localLogout() { if (confirm("هل تريد تسجيل الخروج من حسابك المحلي؟ سيتم مسح اسمك ورقم هاتفك من هذه المتصفح.")) { if (currentUser && currentUserPhone) { endUserSession(currentUser, currentUserPhone); } localStorage.removeItem("falak_username"); localStorage.removeItem("falak_userphone"); localStorage.removeItem("falak_device_id"); localStorage.removeItem("falak_returning_user"); currentUser = null; currentUserPhone = null; updateAdminUI(); location.reload(); } }
  ...
  function applyTheme(theme) { try { if (theme === "light") { document.body.classList.add("light-mode"); } else { document.body.classList.remove("light-mode"); } const icon = document.getElementById("themeToggleIcon"); if (icon) icon.className = theme === "light" ? "fas fa-sun" : "fas fa-moon"; } catch(e) {} }
  function toggleTheme() { try { const next = document.body.classList.contains("light-mode") ? "dark" : "light"; localStorage.setItem("falak_theme", next); applyTheme(next); } catch(e) {} }
  function hideAuthSplash() { const as = document.getElementById("authSplash"); if (as) as.style.display = "none"; }
  ...
  window.addEventListener("load", function() {
    ...
    loadUserDataFromStorage(); hideLoader(); try { applyTheme(localStorage.getItem("falak_theme") || "dark"); } catch(_){} setTimeout(function(){ const as = document.getElementById("authSplash"); if (as && as.style.display !== "none") { hideAuthSplash(); document.getElementById("landingPage").style.display = "flex"; } }, 8000); requestFullscreenAutomatically(); setTimeout(requestFullscreenAutomatically, 1000); initAuthState(); loadEmailSettingsFromFirestore(); try { loadAiKeyOnce(); listenAiKey(); } catch(_){} document.getElementById("googleSignInBtn").onclick = googleLogin; document.getElementById("googleLogoutBtn").onclick = googleLogout; setTimeout(() => updateAdminUI(), 500); setInterval(cleanupOldSessions, 60 * 60 * 1000); });
